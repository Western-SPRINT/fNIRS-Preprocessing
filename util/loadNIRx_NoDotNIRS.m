% modified version of loadNIRx that will not use .nirs
%   some of our older data has errors in the .nirs files

function raw = LoadNIRx_NoDotNIRS(folder,registerprobe)
...
if(isfield(info,'ShortDetectors') && (info.ShortDetectors > 0) && (size(probe.detPos,1) == (info.Detectors - info.ShortDetectors)))
    warning('Adding default SDC positions and connections in order of sources');
...
if(~isempty(file))
    ...
    if(~isfield(info,'Wavelengths') || info.Wavelengths(1)==2)
        % Disabled original (incorrect) defaults
        % % % % not sure why
        % % % info.Wavelengths=[780 850];

        % This section can occur when ALL of the following are met:
        %   1. NIRSport/NIRSport2 device (NIRScout should not encounter this)
        %   2. Fairly recent version of Aurora
        %   3. Loading directly from raw files (not .nirs or .snirf)
        %   4. A .hdr file is present (as opposed to defaulting to use the config.json)

        % Need to determine wavelengths...
        % fprintf("Could not read wavelengths from core files...\n")
        for attempt = 1:3
            try
                switch attempt
                    case 1
                        % .snirf
                        % fprintf("\tAttempting to read wavelengths from .snirf\n");
                        file = dir(fullfile(folder,'*.snirf'));
                        if ~isempty(file)
                            [~,temp] = evalc('nirs.io.loadSNIRF([file.folder filesep file.name], true);');
                            info.Wavelengths = temp.probe.types';
                            % fprintf("\t\tRead wavelengths: [%s]\n", strjoin(string(info.Wavelengths), " , "));
                            break
                        end
    
                    case 2
                        % .nirs
                        % fprintf("\tAttempting to read wavelengths from .nirs\n");
                        file = dir(fullfile(folder,'*.nirs'));
                        if ~isempty(file)
                            [~,temp] = evalc('nirs.io.loadDotNirs([file.folder filesep file.name], true);');
                            info.Wavelengths = temp.probe.types';
                            % fprintf("\t\tRead wavelengths: [%s]\n", strjoin(string(info.Wavelengths), " , "));
                            break
                        end
                    ...
                    case 3
                        % no other option but to assume NIRSport2 defaults
                        info.Wavelengths = [760 850];
                        warning("Could not verify wavelengths! Defaulting to [%s]", strjoin(string(info.Wavelengths), " , "));
    
                    otherwise
                        error("Unexpected attempt number")
                end
            end
        end
    end
    
    probe.link=[repmat(probe.link,length(info.Wavelengths),1) ...
